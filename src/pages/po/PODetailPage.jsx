...
export default function PODetailPage() {
  ...
  //
  // null       = no PDF request
  // preview    = Preview PDF generating
  // download   = Official PDF downloading
  // =====================================================
  ...
  const mutate = useMutation({
    mutationFn: ({ action, payload }) => {
      if (action === "submit") {
        return poApi.submit(id, payload);
      }

      if (action === "approve") {
        return poApi.approve(id, payload);
      }

      if (action === "reject") {
        return poApi.reject(id, payload);
      }

      if (action === "issue") {
        return poApi.issue(id);
      }

      if (action === "cancel") {
        return poApi.cancel(id, payload);
      }

      if (action === "close") {
        return poApi.close(id, payload);
      }

      if (action === "revise") {
        return poApi.revise(id, payload);
      }

      if (action === "regenerate") {
        return poApi.regeneratePdf(id, payload);
      }
    ...
    onSuccess: (r, v) => {
      ...
      if (
        ...
        navigate(
          `/purchase-orders/${r.data._id}/edit`
        );
      }
    },
  ...
  async function downloadAttachment(
    ...
    try {
      const blob =
        await poApi.downloadAttachment(
          id,
          attachment._id
        );
